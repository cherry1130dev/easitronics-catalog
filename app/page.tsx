  ...
  return (
    <div className="min-h-screen bg-[#eaeded] text-[#0f1111] flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 w-full flex-1">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2.5 mb-4 sm:mb-5 bg-white border border-slate-200 p-3 sm:p-4 rounded-xl shadow-sm">
          {/* Left: Project Count Summary */}
          <div className="flex items-center gap-2 text-xs text-slate-700">
            <span className="font-bold text-slate-900">Project Catalog</span>
            <span className="text-slate-400">•</span>
            <span className="text-slate-600">
              Showing <span className="font-bold text-amber-700">{filteredProjects.length}</span> of {allProjects.length} titles
            </span>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-2.5 ml-auto">
            ...
            <div className="hidden md:flex items-center gap-1 bg-slate-100 border border-slate-200 p-1 rounded-lg text-xs">
              <button
                type="button"
                onClick={() => setSortBy('recent')}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-bold text-[11px] transition-all cursor-pointer ${
                  sortBy === 'recent'
                    ? 'bg-[#febd69] text-slate-950 shadow-sm'
            ...
            <div className="flex items-center gap-1.5 bg-slate-100 border border-slate-200 px-2.5 sm:px-3 py-1.5 rounded-lg text-xs">
              <ArrowUpDown className="w-3.5 h-3.5 text-slate-500 shrink-0" />
              <label htmlFor="sortSelect" className="text-slate-600 font-medium hidden sm:inline">
                Sort:
              </label>
              <select
                id="sortSelect"
                ...
                className="bg-transparent text-slate-900 font-semibold outline-none cursor-pointer text-xs"
              >
                <option value="recent" className="bg-white text-slate-900">🔥 Recently Added (First Preference)</option>
                ...
                <option value="title-asc" className="bg-white text-slate-900">🔤 Title: A to Z</option>
              </select>
            </div>
            ...
            <button
              ...
              className="lg:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#febd69] hover:bg-[#f3a847] text-slate-950 font-bold text-xs shadow-sm active:scale-95 transition-all cursor-pointer"
            >
        ...
        {activeFilterChips.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-4 sm:mb-5 p-2.5 sm:p-3 bg-white border border-slate-200 rounded-xl shadow-sm text-xs">
            <span className="text-slate-600 font-medium mr-1 text-[11px] sm:text-xs">Active filters:</span>
            {activeFilterChips.map((chip, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-100 text-slate-800 border border-slate-200 font-medium text-[11px] sm:text-xs"
              >
            ...
            <button
              ...
              className="text-amber-700 hover:text-amber-800 font-bold underline ml-2 transition-colors text-[11px] sm:text-xs cursor-pointer"
            >
              Clear All
            </button>
          </div>
      ...
      {mobileFilterOpen && (
        <div className="fixed inset-0 z-50 flex lg:hidden">
          ...
          />
          <div className="relative ml-auto w-full max-w-sm bg-white border-l border-slate-200 h-full flex flex-col z-10 shadow-2xl animate-in slide-in-from-right duration-200">
            {/* Sticky Drawer Header */}
            <div className="flex items-center justify-between p-4 border-b border-slate-200 bg-slate-50 shrink-0">
              <div className="flex items-center gap-2">
                <SlidersHorizontal className="w-4 h-4 text-amber-600" />
                <span className="font-bold text-slate-900 text-base">Filter Projects</span>
                {activeFilterCount > 0 && (
                  <span className="text-xs px-2 py-0.5 rounded-full bg-[#febd69] text-slate-950 font-bold">
                    {activeFilterCount}
                  </span>
                )}
              </div>
            ...
            <div className="p-4 border-t border-slate-200 bg-slate-50 shrink-0">
              <button
                onClick={() => setMobileFilterOpen(false)}
                className="w-full py-3 bg-[#ffd814] hover:bg-[#f7ca00] text-slate-950 font-black text-sm rounded-xl shadow-md border border-[#fcd200] flex items-center justify-center gap-2 transition-all active:scale-[0.98] cursor-pointer"
              >
...
export default function Home() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#eaeded] text-slate-700 flex items-center justify-center">Loading catalog...</div>}>
      <CatalogContent />
    </Suspense>
  );
}
